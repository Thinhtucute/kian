...
class FSRSReviewService {
  ...
  // Get vocab by entry_id
  static Future<String> getVocabForEntry(int entryId) async {
    try {
      ...
      final headwords = await db.rawQuery('''
        SELECT text, script, position, is_primary
        FROM headword
        WHERE entry_id = ?
      ...
      if (headword != null && headword.isNotEmpty) {
        return "$headword${reading != null ? ' [$reading]' : ''}";
      }
      else if (reading != null) {
        return reading;
      }
      return "Vocab #$entryId";
    } catch (e) {
    ...
    final card = await FSRSRepository.getCard(entryId);
    ...
    // Debug
    kLog('Processing review for entry $entryId: ${await getVocabForEntry(entryId)}');
    ...
    await FSRSRepository.updateCard(
      entryId,
        {
